...
REFERENCEGENOMEPATH=/mnt/beakerdisk/ANTECHINUS/WGS/short-read/reference-based/TassieDevil/refgenome/TasDev/Sarcophilus_harrisii.DEVIL7.0.dna_rm.toplevel.fa
...
# to run: from Rscript ./one_GTF_per_gene.Rscript $REFERENCEGTFPATH
# ... OBVIOUSLY ONLY NEED TO RUN IT ONCE ... 
# it will put the reference genome's GTF files in e.g. /mnt/beakerdisk/ANTECHINUS/WGS/short-read/reference-based/TassieDevil/refgenome/singleGTFs

# remember, some genes are NOT coding, so also output transcripts if we want e.g. annotated long non-coding RNAs
STARTDIR=${PWD}
INPUTGENOME=${SAMPLENAME}.unique_d10_Q30_genome.fasta

cd $STARTDIR
time $OLDSAMTOOLSPATH/samtools faidx $INPUTGENOME # will take a few seconds

mkdir -p ./output
mkdir -p ./output/CDS
mkdir -p ./output/AA
mkdir -p ./output/transcript


#
time for GTF in ${SINGLEGTFSPATH}/*.gtf; do
#
# GTF=ENSSHAG00000000016.gtf # testing
# GTF="./singleGTFs/ENSSHAG00000000008.gtf"
y=${GTF%.gtf}
OUTPUTNAME=$(echo ${y##*/})
echo $OUTPUTNAME # e.g. ENSSHAG00000001841
#
time gffread -x ./output/CDS/${OUTPUTNAME}_CDS.fa -g ${INPUTGENOME} $GTF # be careful here -- add your NEW reference-based genome and the GTF file of your reference
time gffread -y ./output/AA/${OUTPUTNAME}_AA.fa -g ${INPUTGENOME} $GTF # be careful here -- add your NEW reference-based genome and the GTF file of your reference
time gffread -w ./output/transcript/${OUTPUTNAME}_transcript.fa -g ${INPUTGENOME} $GTF # be careful here -- add your NEW reference-based genome and the GTF file of your reference
done
# this will take about two minutes
#
# no transcripts (includes non-coding RNAs)
# no CDS
# no AA

# generate multiFASTAs 
cat ./CDS/*.fa >CDS.mfa && cat ./AA/*.fa >AA.mfa && cat ./transcript/*.fa >transcripts.mfa
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# NOTE ADDED 15.05.19
# replace  N for unknown nucleic acid residue or X for unknown amino acid residue
# with a gap AKA '-' later!
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# use X-filter-gffread-outputV2.R 
